...
class Category extends BaseModel
{
    ...
    {
        $model = new static;
        if (!Cache::get('category_' . $model::$app_id.'_'.$all)) {
            $catModel = $model;
            if(!$all){
                $catModel=$catModel->where('is_show',1);
            }
            $data = $catModel->with(['images'])->order(['sort' => 'asc', 'create_time' => 'asc'])->select();
     ...
     * 获取所有分类
     */
    public static function getCacheAll($all=false)
    {
        return self::getALL($all)['all'];
    }

    /**
     * 获取所有分类(树状结构)
     */
    public static function getCacheTree($all=false)
    {
        return self::getALL($all)['tree'];
    }
