...
class PackageService extends Base
{
    ...
    {
        $package = GiftPackageModel::detail($this->id);
        // 保存目录
        $savePath = $this->getPosterPath($package['app_id']);
        // 删除目录下的文件
        if(!$this->is_empty_dir($savePath)) {
            $this->deleteDir(substr($savePath, 0, -1));
        }
        mkdir($savePath, 0755, true);
        $codeList = CodeModel::getList($this->id);
        if($this->source == 'wx'){
            // 下载小程序码
            $this->saveQrcodeToDir($package['app_id'], 'pages/plus/giftpackage/giftpackage', $savePath, $codeList);
        }else if($this->source == 'mp' || $this->source == 'h5'){
            $this->saveMpQrcodeToDir('h5/pages/plus/giftpackage/giftpackage',$savePath, $codeList, $package['app_id']);
        }

        $zipNameUrl = $this->getZipPath($package['app_id']);
        ...
        if($zip->open($zipNameUrl, \ZipArchive::OVERWRITE) !== TRUE){
            ...
            if ($zip->open($zipNameUrl, \ZipArchive::CREATE) !== true) {
                ...
                throw new BaseException(['msg' => '下载失败，文件夹不存在']);
            }
        }
    ...
    {
        // 保存路径
        return root_path('public') . 'temp' . '/' . $app_id . '/package-' . $this->id . '/' . $this->source. '/';
    }
    ...
    {
        // 保存路径
        return root_path('public') . 'temp' . '/' . $app_id . '/package-' . $this->id . '/' . $this->source. '.zip';
    }
