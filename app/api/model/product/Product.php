...
use app\common\model\product\ProductSite;
...
class Product extends ProductModel
{
    ...
    /**
      *获取打卡清单
     */
    public function getSite($productId)
    {
        $countApply = OrderApply::hasWhere('order',['pay_status'=>20])->where('product_id',$productId)->count();
        $res = ProductSite::where('product_id',$productId)->select();
        foreach ($res as &$value){
            $value["count_apply"] =$countApply;

            $value["count_sign"] =SignInLog::where('site_id',$value['product_site_id'])->count();
        }
        return $res;
    }
    ...
    * 更新打卡时间
     */
    public function setSite($site_id, $time)
    {
        return ProductSite::where('site_id',$site_id)-> update(['sign_in_time'=>$time]);
    }
