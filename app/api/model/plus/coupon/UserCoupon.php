...
class UserCoupon extends UserCouponModel
{
     ...
     * 获取用户优惠券列表
     */
    public function getPageList($user_id, $is_use, $is_expire, $params)
    {
        return $this->with(['coupon'])->where('user_id', '=', $user_id)
     ...
     * 获取用户优惠券列表
     */
    public function getList($user_id, $is_use = 0, $is_expire = 0)
    {
        return $this->with(['coupon'])->where('user_id', '=', $user_id)
            ->where('is_use', '=', $is_use ? 1 : 0)
            ->where('is_expire', '=', $is_expire ? 1 : 0)
     ...
     * @param $user_id
     */
    public function addUserCoupon($coupon_ids, $user)
    {
     ...
     * 礼包购礼优惠券奖励
     */
    public function addNewUserCoupon($coupon_ids, $user_id, $app_id = null)
    {
        $model = new CouponModel();
        $coupon = json_decode($coupon_ids,true);
        foreach ($coupon as $key => $value) {
            //查询优惠券信息
            $couponInfo = $model->where('coupon_id', '=', $value['coupon_id'])->find();
            //计算有效期
            if ($couponInfo['expire_type'] == 10) {
                $start_time = time();
                $end_time = $start_time + ($couponInfo['expire_day'] * 86400);
            } else {
                $start_time = $couponInfo['start_time']['value'];
                $end_time = $couponInfo['end_time']['value'];
            }
             for ($i=0; $i <$value['coupon_num'] ; $i++) { 
                $data[] = [
                    'coupon_id' => $couponInfo['coupon_id'],
                    'name' => $couponInfo['name'],
                    'color' => $couponInfo['color']['value'],
                    'coupon_type' => $couponInfo['coupon_type']['value'],
                    'reduce_price' => $couponInfo['reduce_price'],
                    'discount' => $couponInfo->getData('discount'),
