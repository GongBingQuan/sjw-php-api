...
class Product extends Controller
{
    ...
    public function index()
    {
        ...
        // 商品分类
        $category = CategoryModel::getCacheTree(true);
        // 数量
        $product_count = [
    ...
    public function search()
    {
        // 获取全部商品列表
        $model = new ProductModel;
        $list = $model->getList(array_merge(['status' => -1,'type'=>'all'], $this->postData()));
    ...
    public function lists()
    {
        ...
        // 商品分类
        $catgory = CategoryModel::getCacheTree(true);
     ...
     * 获取产品相关报名人
     */
    public function apply($product_id,$list_rows,$page){
        $list = OrderApply::where('product_id',$product_id)->page($page,$list_rows)->select();
        return $this->renderSuccess('', [
            'list' => [
                'data'=>$list,
                'total'=>OrderApply::where('product_id',$product_id)->count()
            ],
     ...
     *  获取打卡列表
     */
    public function signInLog($product_id,$list_rows,$page){
        $list = OrderApply::where('product_id',$product_id)->page($page,$list_rows)->select();
        return $this->renderSuccess('', [
            'list' => [
                'data'=>$list,
