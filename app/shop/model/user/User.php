...
use app\common\model\user\UserEditLog as UserEditLogModel;
...
class User extends UserModel
{
     ...
     * 获取用户列表
     */
    public static function getList($nickName, $grade_id, $reg_date,$mobile, $gender, $params)
    {
        $model = new static();
        //检索：用户名
        if (!empty($nickName)) {
            $model = $model->whereOr('user.nickName', 'like', '%' . $nickName . '%');
        }
        if(!empty($mobile)){
            $model = $model->where('user.mobile', 'like', '%' . $nickName . '%');
        }
        // 检索：会员等级
        if ($grade_id > 0) {
