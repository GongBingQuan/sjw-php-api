...
class Product extends ProductModel
{
     ...
     * 添加活动打卡清单
     */
    private function addProductSite($sites, $isUpdate = false)
    {
        $site = $this->site()->select()->toArray();
        $ids = array_map(function ($item){
            return $item['site_id']??0;
        },$sites);
        $siteArr = [];
        foreach ($site as  $val){
            if(!in_array($val['site_id'],$ids??[])){
                $this->site()->delete($val['site_id']);
            }
            $siteArr[$val['site_id']] = $val;
        }
